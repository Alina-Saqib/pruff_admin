...
const RecurringEmails = ({ recurringEmails,handleDelete }: any) => {
   ...
   <Table>
     <TableHead sx={{background:"#888"}}>
       <TableRow>
         ...
         <TableCell sx={{fontWeight:600, fontSize:'20px',color:"white"}}>Frequency</TableCell>
     ...
     <TableBody>
       {recurringEmails.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((email: any) => (
    ...
    'No content'
     ...
     <TableCell>{email.frequency ? email.frequency  : "No Frequency"}</TableCell>
     <TableCell>
  {email.attachments ? (
...
export default RecurringEmails
